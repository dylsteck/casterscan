...
interface CastRepliesLiveFeedProps {
    hash: string;
}

export default function CastRepliesLiveFeed({ hash }: CastRepliesLiveFeedProps) {
    const [filter, setFilter] = useState<string>('list');
    const [expanded, setExpanded] = useState<boolean>(false);
    const [page, setPage] = useState<number>(0);
    const { data: casts, loading } = useReplies({ hash: hash });
