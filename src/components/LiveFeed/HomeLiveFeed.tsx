...
interface HomeLiveFeedProps {
    channel?: string;
    hash?: string;
    user?: string;
}

export default function HomeLiveFeed({ channel, hash, user }: HomeLiveFeedProps) {
    const [filter, setFilter] = useState<string>('list');
    const [expanded, setExpanded] = useState<boolean>(false);
    const [page, setPage] = useState<number>(0);
    const { data: casts, loading } = useLatestCasts({ cursor: page });
